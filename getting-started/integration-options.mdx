...
<Steps>
  ...
  <Step title="Monitor Deals">
    Use the API to include `Plans` in your request (e.g. `GET /leads?include=Plans`) to access key insights like deal status
  </Step>
...
### Data Access Capabilities
- **Complete API access** - full `CRUD` permissions on `Leads` and `Read` permission on associated `Plans`
