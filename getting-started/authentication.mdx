...
description: "Learn how to authenticate with the Perch Core API using API keys"
---

To use the Perch Core API, you must authenticate each request with an API key that identifies your application and proves that the request is authorized. The authentication process is simple and stateless — just supply one header with each request.
...
<Note>
The API key header is required for every request to the Core API.
</Note>
...
<Steps>
  ...
  <Step title="Configure your environment">
    Based on your access level, you'll use the appropriate sandbox or production domain. See the [API Overview](/getting-started/overview#domains) for the full list of domains by API type.
...
- Review the [Overview](/getting-started/overview) for general API concepts and best practices
